...
namespace HUSH_RATCHET_DEMO
{
    public partial class WhisperRatchet : Form
    {
        ...
        private void initBtn_Click(object sender, EventArgs e)
        {
            ...
            alice.whisp = new Hush.Whisper();
            alice.whisp.cng = alice.cng;
        ...
        {
            //Alice prepare
            alice.whisp.name = "alice";
            bob.whisper.name = "bob";
            alice.whisp.cng.plaintextBytes = Encoding.ASCII.GetBytes(ain.Text);
            alice.whisp = alice.whisperer.whisper(alice.whisp);
            //Alice Send
            update();
        }
        ...
        private void asend_Click(object sender, EventArgs e)
        {
            ...
            alice.whisp.cng.plaintextBytes = Encoding.ASCII.GetBytes(ain.Text);
            alice.whisp = alice.whisperer.whisper(alice.whisp);
            bob.whisper.bytes = alice.whisp.bytes; //TRANSFER WHISPER
            //Bob Recieved
        ...
        private void update()
        {
            ...
            bkey.Text = Convert.ToBase64String(alice.whisp.cng.key);
            bpk.Text = Convert.ToBase64String(bob.whisper.cng.publicKey);
            apk.Text = Convert.ToBase64String(alice.whisp.cng.publicKey);
            if (bob.whisper.publicKey!= null)
                bobpk.Text = Convert.ToBase64String(bob.whisper.publicKey);
            if (alice.whisp.publicKey != null)
                alicepk.Text= Convert.ToBase64String(alice.whisp.publicKey);
            ...
            if (alice.whisp.ratchet.cng.key != null)
            {
                
                aratchet.Text = Convert.ToBase64String(alice.whisp.ratchet.cng.key);
            }
        }
        private void bsend_Click(object sender, EventArgs e)
        {
            ...
            bob.whisper.cng.plaintextBytes = Encoding.ASCII.GetBytes(bin.Text);
            bob.whisper = bob.whisperer.whisper(bob.whisper);
            alice.whisp.bytes = bob.whisper.bytes; //TRANSFER WHISPER
            //alice Recieved

            //alice parses
            alice.whisp = alice.whisperer.listen(alice.whisp);
            aout.AppendText(Encoding.ASCII.GetString(alice.whisp.bytes));
            //aout.AppendText("\r\n");
            update();
        }
    }
